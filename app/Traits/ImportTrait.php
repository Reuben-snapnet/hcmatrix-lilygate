...
trait ImportTrait
{
        ...
        if ($request->hasFile('template')) {
            ...
            foreach ($datas as $row) {
                ...
                if (!$old_user) {
                    ...
                    $user = User::create([
                        'first_name' => $row['firstname'], 'last_name' => $row['lastname'], 'email' => $row['email'], 'phone' => $row['phone'], 'emp_num' => $row['staffid'], 'sex' => $gender,
                        ...
                        'company_id' => $company->id, 'hiredate' => $row['hire_date'], 'confirmation_date' => $row['date_confirmed'], 'role_id' => 4
                    ]);
                    ...
                    $old_user->update([
                        'first_name' => $row['firstname'], 'last_name' => $row['lastname'], 'email' => $row['email'], 'phone' => $row['phone'], 'sex' => $gender,
                        'address' => $row['address'], 'marital_status' => $row['marital_status'], 'dob' => $row['date_of_birth'], 'branch_id' => ($branch ? $branch->id : 0),

                        'bank_id' => ($bank ? $bank->id : 0), 'status' => $confirmation_status, 'bank_account_no' => $row['account_number'],
                        'company_id' => $company->id, 'hiredate' => $row['hire_date'], 'confirmation_date' => $row['date_confirmed']
                    ]);
        ...
        if ($request->hasFile('template')) {
            ...
            foreach ($datas as $row) {
                ...
                if (!$grade) {
                    Grade::create([
                        'level' => $row->level,
                        'leave_length' => $row->leave_length,
                        'basic_pay' => $row->monthly_gross,
                        'company_id' => $company->id,
    ...
    public function importDependants(Request $request)
    {
        ...
        if ($request->hasFile('template')) {
            ...
            foreach ($datas[0] as $row) {
                ...
                if (!$dependant) {
                    Dependant::create([
                        'name' => $row['name'], 'user_id' => $row['employee_id'], 'email' => $row['email'], 'phone' => $row['phone'],
    ...
    public function downloadEmployeeUploadTemplate(Request $request)
    {
        $first_row = [
            [
                'StaffID' => 'PN12345', 'FirstName' => 'John', 'MiddleName' => 'Robert', 'LastName' => 'Doe', 'Email' => '', 'Gender' => '', 'Date of Birth' => '2000-01-01', 'Phone' => '081234567890', 'Marital Status' => '', 'Address' => '', 'Hiredate' => '2019-01-01', 'Confirmation Status' => '', 'Date confirmed' => '2019-06-01', 'Branch' => '', 'Current Job Role' => '', 'Grade' => '', 'Bank' => '', 'Account Number' => '', 'Next of Kin' => 'Jane Doe', 'Relationship' => '', 'Phone of Next of Kin' => '081234567890', 'Address of Next of Kin' => ''
            ]
        ];
        ...
        $relationship = [
            ['option' => 'spouse', 'value' => 'spouse'], ['option' => 'mother', 'value' => 'mother'], ['option' => 'father', 'value' => 'father'], ['option' => 'son', 'value' => 'son'], ['option' => 'daughter', 'value' => 'daughter'], ['option' => 'brother', 'value' => 'brother'], ['option' => 'sister', 'value' => 'sister'], ['option' => 'nephew', 'value' => 'nephew'], ['option' => 'niece', 'value' => 'niece'], ['option' => 'uncle', 'value' => 'uncle'], ['option' => 'aunt', 'value' => 'aunt'], ['option' => 'friend', 'value' => 'friend']
        ];
        return $this->exportToExcelDropDown(
    ...
    public function downloadAcademicQualificationTemplate(Request $request)
    {
        ...
        return $this->exportToExcelDropDown(
            'Employee Academic Qualification Template',
                                ...
                                if ($data[1] != '') {
                                    ...
                                    $sheet->_parent->addNamedRange(
                                        new \PHPExcel_NamedRange(
                                            "sd{$data[1]}",
                                            $sheet->_parent->getSheet($i),
                                            "B2:B" . $sheet->_parent->getSheet($i)->getHighestRow()
                                        )
                                    );
                                    $i++;
                                    for ($j = 2; $j <= 500; $j++) {
